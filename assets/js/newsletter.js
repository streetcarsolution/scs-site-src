(() => {
  // 👉 Mets ici l’URL /exec de TON déploiement Apps Script (celle qui marche pour toi)
  ...
  document.querySelectorAll('form[data-newsletter]').forEach((form) => {
    ...
    // Où afficher le message (on utilise le <p class="newsletter-msg"> juste après le form)
    let msg = form.parentElement.querySelector('.newsletter-msg');
    if (!msg) {
      msg = document.createElement('p');
      msg.className = 'newsletter-msg mt-2 text-sm';
      form.parentNode.insertBefore(msg, form.nextSibling);
    }

    // Langue : data-lang > <html lang> > fr
    const lang = (form.dataset.lang || document.documentElement.lang || 'fr')
      .slice(0, 2)
      .toLowerCase();
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        ...
        msg.className = 'newsletter-msg mt-2 text-sm text-red-500';
        return;
      }
      ...
      try {
        // GET pour éviter une pré-requête (OPTIONS)
        const url =
        ...
        msg.className = 'newsletter-msg mt-2 text-sm text-green-600';
        input.value = '';
      } catch (err) {
